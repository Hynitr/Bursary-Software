...
if(isset($_POST['pname']) && isset($_POST['clss']) && isset($_POST['fst']) && isset($_POST['snd']) && isset($_POST['trd']) && isset($_POST['adid'])) {
	...
	$der = $_SESSION['aca'];
		...
		$sqll.= " VALUES('$pname', '$new', '$clss' , '$fst', '$snd', '$trd', '$fid', '$der', '$adid')";
		$resullt = query($sqll);
		confirm($resullt);
...
if(isset($_POST['uplpname']) && isset($_POST['uplclss']) && isset($_POST['uplfst']) && isset($_POST['uplsnd']) && isset($_POST['upltrd']) && isset($_POST['upladid']) && isset($_POST['stid'])) {
	...
	$der = $_SESSION['aca'];
		...
		//update student db
		$sqll = "UPDATE student SET `name` = '$uplpname', `fname` = '$new', `class` = '$uplclss', `fst` = '$uplfst', `snd` = '$uplsnd', `trd` = '$upltrd', `adid` = '$adid' WHERE `stid` = '$stid'";
		$resullt = query($sqll);
		confirm($resullt);

		//update fee db
		$ssll = "UPDATE feercrd SET `name` = '$uplpname', `fname` = '$new', `class` = '$uplclss', `adid` = '$adid' WHERE `stid` = '$stid'";
		$ressullt = query($ssll);
		confirm($ressullt);
...
if (isset($_POST['std']) && isset($_POST['trm']) && isset($_POST['fee']) && isset($_POST['cls']) && isset($_POST['mdd']) && isset($_POST['descr']) && isset($_POST['pde']) && isset($_POST['pdat'])) {
	...
	if($fee > $new) {
		...
		$sqlls = "INSERT INTO feercrd(`feeid`, `stid`, `adid`, `amount` , `name`, `fname`, `class`, `session`, `term`, `datepaid`, `mode`, `descr`, `moredecr`)";
		$sqlls.= " VALUES('$fid', '$stid', '$std', '$fee' , '$name', '$new', '$cls', '$red', '$trm', '$date', '$mdd', '$desc', '$pdet')";
		$resullt = query($sqlls);
		confirm($resullt);
